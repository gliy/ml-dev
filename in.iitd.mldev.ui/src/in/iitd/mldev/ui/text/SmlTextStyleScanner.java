...
/**
 * A scanner which scans for SML keywords. It should be run only on document
 * partitions which do not contain comments or strings.
 */
public class SmlTextStyleScanner extends BufferedRuleBasedScanner {
	...
	/**
	 * Creates a scanner that returns a token with the given data for SML
	 * keywords.
	 */
	public SmlTextStyleScanner(Object data, Object[] rainbowParensData) {
		...
		WordRule alphaWords = new WordRule(new AlphaWordDetector(), new Token(
				null));
	...
	public IToken nextToken() {
		...
		if (nextToken == OPEN_PAREN_TOKEN) {
			Object data = rainbowParensData[currentParenLevel
					% rainbowParensData.length];
			currentParenLevel++;
